...
# nav_option = st.sidebar.radio("Go to", ['About', 'Narcotics & CBI'])

# # --------------- ABOUT SECTION ---------------
# if nav_option == 'About':
#     st.title("About This Tool")
#     st.write("""
#     Welcome to the **WhatsApp Chat Analyzer**. This tool allows you to analyze WhatsApp chats 
#     for various metrics like total messages, most active users, and activity maps.
...
#     - View flagged messages after clicking on the sensitive information button
...
#         # Assuming preprocessor function converts WhatsApp chat into DataFrame with 'user' and 'message' columns
#         df = pd.DataFrame({'user': ['Nic', 'Bhavya Verma', 'John', 'Nic'], 
#                            'message': ['Let\'s go for this na', 'Bring 2g of stuff', 'Send payment via UPI', 'When is my birthday?']})
#         st.dataframe(df)
...
#     # Predefined list of sensitive words (related to narcotics, finance, cybercrime, and threats)
#     sensitive_words = [
...
#     # Function to detect sensitive words in the messages
...
#             if match:  # If a sensitive word is found
#                 flagged_messages.append({
#                     'user': row['user'],
#                     'message': row['message'],
#                     'bad_word': match.group()  # The specific word that matched
#                 })
...
#         # Identify the user with the most flagged messages
#         culprit = flagged_df['user'].value_counts().idxmax()
#         st.warning(f"🚨 **Potential Culprit Detected: {culprit}** 🚨")
            
#         st.warning("The following table contains sensitive information. Click the button below to view it.")
            
#         if st.button("View Sensitive Information"):
#             with st.spinner('Loading sensitive information, please wait...'):
#                 st.dataframe(flagged_df[['user', 'message', 'bad_word']])
#     else:
#         st.write("No sensitive information detected in the chat.")

#     # Upload bad words dataset
#     # Using readlines() to load bad words file
#     bad_words_file = st.sidebar.file_uploader("Upload Bad Words Dataset (CSV or TXT)")
#     if bad_words_file is not None:
#         if bad_words_file.name.endswith('.txt'):
#             bad_words_list = bad_words_file.getvalue().decode('utf-8').splitlines()
#         elif bad_words_file.name.endswith('.csv'):
#             bad_words_df = pd.read_csv(bad_words_file, header=None, names=['bad_word'])
#             bad_words_list = bad_words_df['bad_word'].str.strip().tolist()
        
#         st.write(f"Loaded {len(bad_words_list)} bad words for analysis.")
...
#         if bad_words_file is not None:
#             st.title("Bad Word Detection")
            
#             flagged_df = detect_bad_words(df, bad_words_list)

#             if not flagged_df.empty:
#                 st.write(f"Total flagged messages with bad words: {len(flagged_df)}")
                
#                 st.warning("The following table contains sensitive information. Click on the dropdown below to view it.")
#                 #st.dataframe(flagged_df[['user', 'message', 'bad_word']])
#                 with st.expander("View Sensitive Information"):
#                     st.dataframe(flagged_df[['user', 'message', 'bad_word']])
#                 # if st.button("View Sensitive Information"):
#                 #     with st.spinner('Loading sensitive information, please wait...'):
#                 #         st.dataframe(flagged_df[['user', 'message', 'bad_word']])  # Show only user, message, bad_word columns
#             else:
#                 st.write("No bad words detected in the chat.")
